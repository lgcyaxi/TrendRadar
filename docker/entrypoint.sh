...
check_and_migrate_db() {
    ...
    if [ "$old_db_count" -gt 0 ]; then
        ...
        # 运行迁移脚本（无论 current.db 是否存在，都尝试迁移以合并数据）
        ...
        # 迁移后清理旧格式数据库文件
        echo "🧹 清理旧格式数据库文件..."
        find "${db_dir}/news" -maxdepth 1 -type f -name "????-??-??.db" -delete 2>/dev/null || true
        find "${db_dir}/rss" -maxdepth 1 -type f -name "????-??-??.db" -delete 2>/dev/null || true
        echo "✅ 旧格式文件清理完成"
    else
        echo "✅ 数据库格式检查完成（无需迁移）"
    fi
