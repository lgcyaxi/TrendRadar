...
class ExtensionManager:
    ...
    def apply_notification_send(
        self,
        channel: str,
        context: Any,
    ) -> Optional[bool]:
        """
        Try to handle notification send via NotificationEnhancer plugins.

        If any plugin's send() method returns True or False, that result
        is returned and the dispatcher should skip its default send logic.
        If all plugins return None, the dispatcher proceeds with default logic.
        ...
        for plugin in self.plugins.values():
            if isinstance(plugin, NotificationEnhancer):
                try:
                    if plugin.enabled:
                        config = self.load_plugin_config(plugin.name)
                        result = plugin.send(channel, config, context)
                        if result is not None:
                            return result
                except Exception as e:
                    logger.error("[Extension] Error in {}.send(): {}", plugin.name, e)
