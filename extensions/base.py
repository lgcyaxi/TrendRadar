...
class NotificationEnhancer(ExtensionPoint):
    ...
    - Complete channel takeover (handle send entirely)
    ...
    def send(
        ...
        """
        Optional: Handle the entire send operation for a channel.

        If this method returns True or False, the dispatcher should skip
        its default send logic. If this returns None, the dispatcher
        proceeds with its default send logic.

        This allows plugins to completely take over a channel's notification
        handling, including formatting, mock mode, and actual sending.
