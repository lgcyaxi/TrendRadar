...
│   ├── notification_scheduler/   # Notification scheduling plugin
│   │   └── __init__.py           # Plugin class
│   └── wework_compact/           # WeWork compact format plugin
│       └── __init__.py           # Plugin class
└── trendradar/
...
| `notification_scheduler` | Schedule notifications by time        | `notification_scheduler.yaml` | Enabled by default  |
| `wework_compact`         | WeWork compact message format         | `wework_compact.yaml`         | Enabled by default  |

> **Note:** AI analysis for HTML reports is now built into the core (`trendradar/report/html.py`) since v5.2.0. The previous `html_ai_analysis` extension has been deprecated and removed.
...
| `extensions/notification_scheduler/` | Scheduling plugin    | 调度插件       |
| `extensions/wework_compact/`     | WeWork format plugin     | 企微格式插件   |
